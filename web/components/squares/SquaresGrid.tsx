...
const SquaresGrid = ({ gameState, setPurchaseStatus }) => {
        ...
        <div>
          ...
          <div className="flex">
            ...
            {Array.from({ length: gridSize }).map((_, index) => (
              <div key={index} className="w-10 h-10 flex justify-center items-center border border-gray-500">?</div>
            ))}
          </div>
  ...
  return (
    ...
    >
      ...
      {isBoardFilled ? (
  <button className="btn btn-primary mt-4 ml-40" disabled={!isCreator}>
    Finalize Board
  </button>
) : (
  <button
